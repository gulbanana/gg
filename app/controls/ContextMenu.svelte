...
<div
    ...
    {:else if operand.type === "Ref" && refEnabled}
        ...
        <hr />
        <button disabled={!refEnabled.push_all} on:click={() => onClick("push-single")}>Push</button>
        <button disabled={!refEnabled.fetch_all} on:click={() => onClick("fetch-single")}>Fetch</button>
        <hr />
