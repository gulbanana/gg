...
impl AppState {
    ...
    pub fn request_shutdown(&self) {
        ...
        let handle = tokio::spawn(async move {
            // Grace period: wait 3 seconds before actually shutting down
            // This allows a reload to cancel the shutdown
            tokio::time::sleep(Duration::from_secs(3)).await;
            log::info!("Shutdown grace period elapsed, shutting down");
        ...
        *self.pending_shutdown.lock().unwrap() = Some(handle);
        log::debug!("Shutdown intent received, starting 3s grace period");
    }
