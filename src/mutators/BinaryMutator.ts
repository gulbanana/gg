...
export type RichHint = (
    ...
    | ChangeId
    | CommitId
    | Extract<StoreRef, { type: "LocalBookmark" } | { type: "RemoteBookmark" }>
)[];
export type Eligibility =
    | { type: "yes"; hint: RichHint }
    | { type: "maybe"; hint: string }
    | { type: "no" };
...
export default class BinaryMutator {
    ...
    canDrop(): Eligibility {
        ...
        if (
            BinaryMutator.canDrag(this.#from).type != "yes" &&
            !(this.#from.type == "Revision" && this.#to.type == "Merge")
        ) {
            return { type: "no" };
        ...
        if (this.#from.type == "Revision") {
            if (this.#to.type == "Revision") {
                return {
                    ...
                    hint: [
                        "Rebasing revision ",
                        this.#from.header.id.change,
                        " onto ",
                        this.#to.header.id.change,
                    ],
                };
                ...
                if (this.#to.header.id.change.hex == this.#from.header.id.change.hex) {
                    ...
                    return {
                        type: "yes",
                        hint: ["Adding parent to revision ", this.#to.header.id.change],
                    };
                }
        ...
        if (this.#from.type == "Parent") {
            if (this.#to.type == "Repository") {
                return {
                    type: "yes",
                    hint: ["Removing parent from revision ", this.#from.child.id.change],
                };
            }
        }
            ...
            if (this.#to.type == "Revision") {
                if (this.#to.header.id.change.hex == this.#from.header.id.change.hex) {
                    ...
                    return {
                        type: "yes",
                        hint: [
                            `Squashing changes at ${this.#from.path.relative_path} into `,
                            this.#to.header.id.change,
                        ],
                    };
                }
                ...
                if (this.#from.header.parent_ids.length == 1) {
                    return {
                        type: "yes",
                        hint: [
                            `Restoring changes at ${this.#from.path.relative_path} from parent `,
                            this.#from.header.parent_ids[0],
                        ],
                    };
                } else {
        ...
        if (this.#from.type == "Ref" && this.#from.ref.type != "Tag") {
            ...
            // remote -> local: track
                ...
                this.#to.type == "Ref" &&
                this.#to.ref.type == "LocalBookmark" &&
                this.#from.ref.type == "RemoteBookmark" &&
                this.#from.ref.branch_name == this.#to.ref.branch_name
            ) {
            ...
            // anything -> anywhere: delete
                ...
                if (this.#from.ref.type == "LocalBookmark") {
                    ...
                    return {
                        type: "yes",
                        hint: ["Forgetting remote bookmark ", this.#from.ref],
                    };
                }
            }
        }
    ...
    doDrop() {
        if (this.#from.type == "Revision") {
            if (this.#to.type == "Revision") {
                ...
                mutate<MoveRevision>("move_revision", {
                    id: this.#from.header.id,
                    parent_ids: [this.#to.header.id],
                });
                return;
            } else if (this.#to.type == "Parent") {
                // rebase between targets
                mutate<InsertRevision>("insert_revision", {
                    id: this.#from.header.id,
                ...
                mutate<MoveSource>("move_source", {
                    id: this.#to.header.id,
        ...
        if (this.#from.type == "Parent") {
            if (this.#to.type == "Repository") {
                ...
                let newParents = this.#from.child.parent_ids.filter(
                    (id) => id.hex != removeCommit.hex
                );
                mutate<MoveSource>("move_source", {
                    id: this.#from.child.id,
                    parent_ids: newParents,
                });
                return;
            }
