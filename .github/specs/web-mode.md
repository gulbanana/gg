...
2. **Heartbeat timeout**: Frontend sends a heartbeat every 30 seconds via `/api/trigger/heartbeat`. Backend shuts down after 2 minutes without a heartbeat (handles browser crash or tab close when beacon fails).

### Crash Detection
The heartbeat also enables frontend detection of backend crashes. If 2 consecutive heartbeats fail (network error, server down), the frontend sets `repoConfigEvent` to `WorkerError` state, showing the fatal error dialog and stopping further backend communication attempts.
